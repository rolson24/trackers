...
class AnchorToPositiveDistanceMetric(TripletMetric):
    """
    ...
    def __str__(self):
        return "anchor_to_positive_distance"
    ...
    def update(
        ...
        dist_ap = F.pairwise_distance(anchor_embed, positive_embed, p=2)
        self.distance += torch.sum(dist_ap).item()
...
class AnchorToNegativeDistanceMetric(TripletMetric):
    """
    ...
    def __str__(self):
        return "anchor_to_negative_distance"
    ...
    def update(
        ...
        dist_an = F.pairwise_distance(anchor_embed, negative_embed, p=2)
        self.distance += torch.sum(dist_an).item()
